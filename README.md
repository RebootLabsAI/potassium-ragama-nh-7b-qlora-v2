...
# Banana.dev ragama-nh-7b-qlora-v2 starter template

This is a ragama-nh-7b-qlora-v2 starter template from [Banana.dev](https://www.banana.dev) that allows on-demand serverless GPU inference.
...
1. Connect your Github account on Banana.
2. [Create a new model](https://app.banana.dev/deploy) on Banana from the forked Github repository.
...
Build the model as a Docker image. You can change the `ragama-nh-7b-qlora-v2` part to anything.

```sh
docker build -t ragama-nh-7b-qlora-v2 .
```

Run the Potassium server

```sh
docker run --publish 8000:8000 -it ragama-nh-7b-qlora-v2
```
